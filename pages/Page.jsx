...
export default function Landing() {
    ...
    return (
        <div className='w-full px-2 py-2 flex flex-col items-center'>
            <h1 className='text-lg font-extrabold py-2 mb-2 border-b border-blue-600'>Shopping Cart</h1>
            <div className='fixed top-0 right-0 '>
                <Link href="#cart" className='relative'><BsCart className=" md:text-6xl mx-4  text-2xl" /></Link>
                <p className='absolute z-10 top-0 right-1 text-green-600 text-xl font-semibold'>{cart.length}</p>
            </div>
                ...
                <div id='cart' className='lg:w-4/12 w-full py-4 px-2'>
                    {
                        cart?.map((prod) => {
                            return (
                                <div key={prod?.id} className='w-full bg-blue-100  py-2 mb-2 flex items-center justify-around'>
